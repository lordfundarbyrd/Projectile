...
public class Projectile extends PApplet {
    ...
    public void balls() {
        if (beg != end) {
            for (int i = 0; i < size; i++) {
                balls.add(new Ball(this, 0, 26, (float) ThreadLocalRandom.current().nextDouble(beg, end)));
            }
        }
        if (beg == end) {
            last = true;
            for (int i = 0; i < size; i++) {
                balls.add(new Ball(this, 0, 26, (float) beg));
            }
        }
    }
    ...
    public void draw() {
        ...
        fill(0);
        textSize(30);
        text("Height: " + Ball.h + " m", width - 500, height / 2 + 90);
        text("Velocity: " + Ball.initVel + " m/s", width - 500, height / 2);
        text("Range: " + length + " m", width - 500, height / 2 - 90);
        text("Angle: " + angle + " deg", width - 500, height / 2 - 180);
