...
public class Ball {
    ...
    public Ball(PApplet pa, int x, int y, float ang, float wd) {
        parent = pa;
        ...
        force = new PVector();
        drag = new PVector();
        wind = new PVector(wd,0,0);
        dragVel = new PVector();
        magnus = new PVector();
        spin = new PVector(0,0,100);
    ...
    public void magnus() {
        magnus = spin.cross(dragVel);
        magnus.mult(s);
        force.add(magnus);
    }
    
    public void drag() {
        dragVel.set(PVector.sub(vel,wind));
